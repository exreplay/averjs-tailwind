...
export default async function() {
  ...
  if (!await exists(resolve(process.env.PROJECT_PATH, './tailwind.css'))) {
    await copy(
      resolve(__dirname, './templates/tailwind.css'),
      resolve(process.env.PROJECT_PATH, './tailwind.css')
    );
  }
  ...
  this.config.purgecss.defaultExtractor = defaultExtractor;
}
